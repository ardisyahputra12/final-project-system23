...
def create_order(current_user):
    ...
    def order(val):
        ...
        shipping_method[0]["price"] = int(regular_price + total_price)

        next_day_price = (20*total_price)/100 if total_price < 300 else (25*total_price)/100
        shipping_method[1]["price"] = int(next_day_price + total_price)
        ...
        if shipping_method[val]["price"] > run_query(select(Users.balance).where(Users.id==current_user))[0]['balance']:
            ...
            run_query(insert(Orders).values(products=products_result,id=uuid.uuid4(),user_id=current_user,shipping_method=method,shipping_address=address,total_price=shipping_method[val]["price"],create_at=format_datetime(),create_by=user_name),True)
...
def get_orders(current_user):
    ...
    for x in run_query(select(Users).where(Users.is_admin==False)):
        for z in run_query(select(Orders).where(Orders.user_id==x['id']).group_by(Orders.id)):
            data.append({"id":z['id'],"user_name":x['name'],"created_at":z['create_at'],"user_id":x['id'],"user_email":x['email'],"total": z['total_price']})
